...
export default function TicketPreview({ ticketId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      ...
      {/* Modal */}
      <div className="bg-[var(--bg-surface)] rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border)]">
          <h2 className="font-semibold text-[var(--text-primary)]">Ticket de venta</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {loading && (
            <p className="text-center text-[var(--text-secondary)] py-8">Cargando ticket...</p>
          )}
          {error && (
          ...
          {ticket && !loading && (
            <>
              ...
              {/* Panel IP impresora */}
              {showIpPanel && (
                <div className="mt-4 p-4 bg-[#f8fafc] rounded-xl border border-[var(--border)]">
                  <p className="text-xs font-semibold text-[var(--text-secondary)] mb-2">IP de la impresora térmica</p>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-[var(--border)] rounded-lg text-sm text-black outline-none focus:border-[#2563eb]"
                    />
                    <button
          ...
          <div className="px-5 py-4 border-t border-[var(--border)] flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={pdfLoading}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-[#2563eb] text-white hover:bg-[var(--accent-hover)] disabled:opacity-50 transition-colors"
              >
                {pdfLoading ? 'Generando...' : 'Descargar PDF'}
              </button>
              <button
                onClick={() => setShowIpPanel(v => !v)}
                className="flex-1 py-2.5 rounded-xl text-sm font-semibold border border-[var(--border)] text-[var(--text-primary)] hover:bg-[var(--bg-page)] transition-colors"
              >
                Imprimir ticket
              </button>
            </div>
            <button
              ...
              className="w-full py-2.5 rounded-xl text-sm font-semibold border border-[var(--border)] text-[var(--text-secondary)] hover:bg-[var(--bg-page)] transition-colors"
            >
